...
def tta(args, images):
    ...
    for i in range(n1):
        for j in range(n2):
            ...
            ret.append(
                nn.functional.interpolate(imgs, images.size()[2:], mode="nearest")
            )
            k += 1
...
def get_train_val_loader(args, predict=False):
    ...
    def train_transform2(image):
        a, b = (
            np.random.normal(1, args.pw_aug[0], (6, 1, 1)),
            np.random.normal(0, args.pw_aug[1], (6, 1, 1)),
        )
        a, b = (
    ...
    if not predict:
        train_dataset = CellularDataset(
            args.data,
            "train_all_controls" if args.all_controls_train else "train_controls",
...
def get_test_loader(args, exclude_leak=False):
    test_dataset = CellularDataset(
        args.data,
        "test" if not exclude_leak else "test_noleak",
        normalization=args.data_normalization,
    )
    return DataLoader(
        test_dataset,
...
class CellularDataset(Dataset):
    ...
    def __init__(
            ...
            if not all_controls:
                for row in pd.read_csv(self.root / "kaggle_test_controls.csv").iterrows():
                    ...
                    if r.experiment == "HUVEC-18":
                        ...
                        self.data.append(
                            (r.experiment, r.plate, r.well, 2, typ, r.sirna)
                        )
        if exclude_leak:
        ...
        if mode != "test":
            ...
            for i in range(len(cells)):
                ...
                if i == 3:
                    cells[i] = (
                        cells[i] + cells[i]
                    )  # duplicate U2OS experiments for validation
            ...
            for count in counts:
                splits.append(
                    sorted(cells[0][start[0] : start[0] + count[0]])
        ...
        controls = list(
            filter(
                lambda d: d[-1] is not None and d[-1] >= self.treatment_classes,
                (all_data if all_controls else self.data),
            )
        )
        self.data = list(
            filter(
                lambda d: not (d[-1] is not None and d[-1] >= self.treatment_classes),
                self.data,
            )
        )
        if move_controls:
    ...
    def __getitem__(self, i):
        ...
        if self.normalization == "experiment":
            ...
            pixel_mean = (
                torch.tensor(list(map(lambda x: x[0], P.pixel_stats.values()))).mean(0)
                / 255
            )
            pixel_std = (
                torch.tensor(list(map(lambda x: x[1], P.pixel_stats.values()))).mean(0)
                / 255
            )
        ...
        cell_type = nn.functional.one_hot(
            torch.tensor(self.cell_types.index(d[-2]), dtype=torch.long),
            len(self.cell_types),
        ).float()
