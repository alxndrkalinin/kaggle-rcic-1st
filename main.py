...
def parse_args():
    ...
    parser.add_argument(
        "-m", "--mode", default="train", choices=("train", "val", "predict")
    )
    parser.add_argument(
        "--backbone",
        default="mem-densenet161",
    ...
    parser.add_argument(
        "--head-hidden",
        type=lambda x: None if not x else list(map(int, x.split(","))),
    ...
    parser.add_argument(
        "--tta-size", type=float, default=1, help="crop percentage for TTA"
    )
    ...
    parser.add_argument(
        "--data-normalization",
        choices=("global", "experiment", "sample"),
        default="sample",
    ...
    parser.add_argument(
        "--num-data-workers", type=int, default=10, help="number of data loader workers"
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def infer(args, model, loader):
    ...
    for i, (X, S, I, *_) in enumerate(loader):
        ...
        if (i + 1) % args.disp_batches == 0:
            logging.info(
                "Infer Iter: {:4d}  ->  speed: {:6.1f}".format(
...
def score(args, model, loader):
    ...
    acc = (
        (cell_type_s.sum() / cell_type_c.sum()).item() if cell_type_c.sum() != 0 else 0
    )
...
def get_learning_rate(args, epoch):
    ...
    for start, end, lr, next_lr in zip(
        [0] + args.lr[1][1::2],
        ...
        if start <= epoch < end:
            if args.lr[0] == "cosine":
                return (
                    lr * (math.cos((epoch - start) / (end - start) * math.pi) + 1) / 2
                )
...
def smooth_label(args, Y):
    ...
    nY[range(Y.size(0)), Y] -= (
        args.label_smoothing / (args.classes - 1) + args.label_smoothing
    )
    return nY
...
def transform_input(args, X, S, Y):
    ...
    if args.mixup != 0:
        coeffs = torch.tensor(
            np.random.beta(args.mixup, args.mixup, args.batch_size), dtype=torch.float32
        ).cuda()
        X = (
            coeffs.view(-1, 1, 1, 1) * X + (1 - coeffs.view(-1, 1, 1, 1)) * X[perm,]
        )
        S = (
            coeffs.view(-1, 1) * S + (1 - coeffs.view(-1, 1)) * S[perm,]
        )
        Y = (
            coeffs.view(-1, 1) * Y + (1 - coeffs.view(-1, 1)) * Y[perm,]
        )
...
def pseudo_label(args, epoch, pl_data, model, val_loader, test_loader, train_loader):
    ...
    for k, (i, v) in itertools.chain(
        zip(itertools.repeat("val"), val_res), zip(itertools.repeat("test"), test_res)
    ):
    ...
    val_test_examples = (
        len(val_loader.dataset.data) // 2 + len(test_loader.dataset.data) // 2
    )
    added_examples = len(pl_data) // 2
    n = (
        round(eval("lambda x: " + args.pl_size_func)(x) * val_test_examples)
        - added_examples
    )
    n = max(n, 0)
    ...
    logging.info(
        "Pseudo-labeling: Added {} ({} val, {} test), {} ({:.3f}%) val misclassified, "
        "{} ({:.3f}%) not added, pl_data size {}, train size {}, threshold {}".format(
            n,
            val_count,
            test_count,
            val_misclass,
            val_misclass / val_count * 100 if val_count != 0 else 0,
            not_added_count,
            not_added_count / (not_added_count + n) * 100
            if not_added_count + n != 0
            else 0,
            len(pl_data),
            len(train_loader.dataset.data),
            confs[-1][1] if len(confs) != 0 else "None",
        )
    )
